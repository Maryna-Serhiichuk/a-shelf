...
export const ProductPreview: FC<Product & { className?: string }> = ({ documentId, illustration, name, price, discount, volume, className, isCart, isOutOfStock }) => {
    ...
    return <div key={documentId} className={classNames(
        "flex flex-col justify-between px-3 sm:px-6 py-3 sm:py-6 bg-stone-100 dark:bg-stone-700",
        className,
        { "opacity-40": isOutOfStock }
    )}>
        <div className="flex items-end justify-center h-[120px] sm:h-[200px]">
            {illustration?.url &&
        ...
        <div>
            <NavLink href={`/product/${name?.replaceAll(' ', '-')}/${documentId}`}>
                <div className="py-3 text-stone-950 dark:text-stone-200 flex justify-center text-md sm:text-xl text-center hover:underline cursor-pointer">
